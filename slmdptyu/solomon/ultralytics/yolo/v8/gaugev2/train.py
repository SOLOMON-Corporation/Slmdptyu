...
from slmdptyu.solomon.ultralytics.nn.tasks import Gaugev2SolModel
from slmdptyu.solomon.ultralytics.yolo.utils import DEFAULT_CFG

# BaseTrainer python usage
class Gaugev2SolTrainer(DetectionSolTrainer):

    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        if overrides is None:
            overrides = {}
        overrides['task'] = 'gaugev2'
        super().__init__(cfg, overrides, _callbacks)

    def get_model(self, cfg=None, weights=None, verbose=True):
        model = Gaugev2SolModel(
            cfg, ch=3, nc=self.data['nc'], 
            with_angle=self.data['with_angle'], 
            data_pointer_shape=self.data['pointer_shape'], 
            data_scaler_shape =self.data['scaler_shape'], 
            data_pointer_idx =self.data['pointer_idx'], 
            verbose=verbose
        )
        if weights:
            model.load(weights)
    ...
    def set_model_attributes(self):
        ...
        self.model.pointer_shape = self.data['pointer_shape']
        self.model.scaler_shape = self.data['scaler_shape']
        self.model.pointer_idx = torch.tensor(self.data['pointer_idx'], device=self.model.model[0].conv.weight.device)

    def get_validator(self):
        if self.args.train_angle:
            self.loss_names = 'box_loss', 'pointer_loss', 'scaler_loss', 'kobj_loss', 'cls_loss', 'dfl_loss', 'ang_loss'
        else:
            self.loss_names = 'box_loss', 'pointer_loss', 'scaler_loss', 'kobj_loss', 'cls_loss', 'dfl_loss'
        return PoseSolValidator(self.test_loader, save_dir=self.save_dir, args=copy(self.args))

    def criterion(self, preds, batch):
        if not hasattr(self, 'compute_loss'):
            self.compute_loss = Gaugev2Loss(de_parallel(self.model))
        return self.compute_loss(preds, batch)


# Criterion class for computing training losses
class Gaugev2Loss(Loss):

    def __init__(self, model):  # model must be de-paralleled
        super().__init__(model)
        self.pointer_shape = model.model[-1].pointer_shape
        self.scaler_shape = model.model[-1].scaler_shape
        self.kpt_shape = (self.pointer_shape[0] + self.scaler_shape[0], 3)
        self.pointer_idx = model.model[-1].pointer_idx
    ...
    def __call__(self, preds, batch):
        ...
        if self.with_angle:
            loss = torch.zeros(7, device=self.device)  # box, cls, dfl, pointer_location, scaler_location, kpt_visibility, angle
            ...
            loss = torch.zeros(6, device=self.device)  # box, cls, dfl, pointer_location, scaler_location, kpt_visibility
        ...
        if self.with_angle:
            targets = torch.cat(
                (
                    ...
                    batch['cls'].view(-1, 1), 
                    batch['bboxes'],
                    torch.cat(self.transAngle_arr_tensor(batch['angles'].flatten()), 1),
                    batch['angles_valid']
                ), 1)
        ...
        if fg_mask.sum():
            ...
            for i in range(batch_size):
                if fg_mask[i].sum():
                    ...
                    kpt_mask = gt_kpt[..., 2] != 0
                    loss[1] += self.keypoint_loss(pred_kpt[:,self.pointer_idx,:], gt_kpt[:,self.pointer_idx,:], kpt_mask[:,self.pointer_idx], diagonal)  # pose loss
                    loss[2] += self.keypoint_loss(pred_kpt[:,~self.pointer_idx,:], gt_kpt[:,~self.pointer_idx,:], kpt_mask[:,~self.pointer_idx], diagonal)  # pose loss
                    # kpt_score loss
                    if pred_kpt.shape[-1] == 3:
                        loss[3] += self.bce_pose(pred_kpt[..., 2], kpt_mask.float())  # keypoint obj loss
        ...
        loss[3] *= self.hyp.kobj / batch_size  # kobj gain
...
def train(cfg=DEFAULT_CFG, use_python=False):
    ...
    if use_python:
        ...
        trainer = Gaugev2SolTrainer(overrides=args)
        trainer.train()
